...
void Biquad_processChannel(Biquad* x, int type, float frequency, float Q, float gain, float sampleRate, float* in,
                           float* out, int n, int channel) {
    ...
    while (n--) {
        ...
        // apply filter
        *out++ = y0 =
            x->b0_over_a0 * x0 + x->b1_over_a0 * x1 + x->b2_over_a0 * x2 - x->a1_over_a0 * y1 - x->a2_over_a0 * y2;
...
void Biquad_process(Biquad* x, int type, float frequency, float Q, float gain, float sampleRate, float* in, float* out,
                    int n) {
