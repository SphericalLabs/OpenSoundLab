...
static float masterBusLimiterCoeff(float t, float samplerate) {
    float smpls = (t / 1000) * samplerate;
    return powf((1 - one_minus_oneOverE), (1.0 / smpls));
}

void MasterBusLimiter_Process(float buffer[], float sc[], int length, int channels, MasterBusLimiterData* x) {
    ...
    while (n--) {
        ...
        delay = (lookaheadSmpls == x->d_prev) ? lookaheadSmpls
                                              : (int) roundf(x->aD * x->d_prev + (1.0f - x->aD) * lookaheadSmpls);
        ...
        for (int k = 0; k < channels; k++) {
            ...
            /// If we are in normal compressor mode, we want to look only at the sidechain signal. In the most common
            /// usecase of a compressor, the sidechain signal is the same signal as the audio signal to process. Using a
            /// distinct signal as a sidechain (for example a kick drum) to compress another signal allows for the
            /// typical "ducking" effect used in modern electronic music.
            else
                xG = _atodb(_max(fabs(sidechain[k]), fabs(x->buf[readPtr + k])));
            /// On the other hand, if we are in limiter mode, we look at current sidechain value as well as current
            /// input signal value, and use whichever is bigger to calculate the gain reduction. If we would only use
            /// the sidechain, then transients that are shorter than the lookahead time will not get attenuated.
            ...
            /// signal is within knee, attenuate smoothly:
                ...
                yG = xG + (1 / ratio - 1) * powf(xG - threshold + knee / 2, 2) / (2 * knee);
            ...
            /// If compressor is in bypass state, we do not apply the makeup gain, and the output is simply the input
            /// signal.
...
struct MasterBusLimiterData* MasterBusLimiter_New(float sampleRate) {
    struct MasterBusLimiterData* x = (struct MasterBusLimiterData*) _malloc(sizeof(struct MasterBusLimiterData));
    ...
    x->aA = 0;
    x->aR = 0;
    x->aD = masterBusLimiterCoeff(DELAY_ATTACK, sampleRate);
    x->y1_prev[0] = 0;
    x->yL_prev[0] = 0;
    x->y1_prev[1] = 0;
    x->yL_prev[1] = 0;
    ...
    printv("Created new MasterBusLimiter instance with sampleRate %f\n", sampleRate);

    return x;
}

void MasterBusLimiter_Free(struct MasterBusLimiterData* x) {
    free(x->buf);
    free(x->params);
    free(x);
}

void MasterBusLimiter_SetParam(float value, int param, struct MasterBusLimiterData* x) {
    assert(param < P_N);

    switch (param) {
    case P_ATTACK:
        if (value != x->params[param])
            x->aA = masterBusLimiterCoeff(value, x->sampleRate);
        break;
    case P_RELEASE:
        if (value != x->params[param])
            x->aR = masterBusLimiterCoeff(value, x->sampleRate);
        break;
    case P_LOOKAHEAD:
...
float MasterBusLimiter_GetAttenuation(struct MasterBusLimiterData* x) {
    return x->attenuation;
}

bool MasterBusLimiter_IsRamping(struct MasterBusLimiterData* x) {
    return (_mstosmpls(x->params[P_LOOKAHEAD], x->sampleRate) == x->d_prev);
}

bool MasterBusLimiter_IsClipping(struct MasterBusLimiterData* x) {
    return x->clipping;
}
