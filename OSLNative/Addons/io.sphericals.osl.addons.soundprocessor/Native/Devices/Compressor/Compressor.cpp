...
OSL_API void Compressor_Process(float buffer[], float sc[], int length, int channels, CompressorData* x) {
    ...
    float makeupTarget = x->params[P_MAKEUP];
    float makeup;
    ...
    while (n--) {
        makeup = x->aM * x->makeup_prev + (1.0f - x->aM) * makeupTarget;
        x->makeup_prev = makeup;
...
OSL_API struct CompressorData* Compressor_New(float sampleRate) {
    ...
    x->aM = _coeff(MAKEUP_ATTACK, sampleRate);
    x->makeup_prev = params[P_MAKEUP];
    x->y1_prev[0] = 0;
    x->yL_prev[0] = 0;
    x->y1_prev[1] = 0;
    x->yL_prev[1] = 0;
