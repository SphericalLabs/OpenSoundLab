...
///  input signal. The compressor has an optional Lookahead feature, which is mainly useful in conjunction with
...
struct CompressorData {
    ...
    float aM; // makeup smoothing coefficient
    float makeup_prev;
    float y1_prev[2];
    float yL_prev[2];
...
/// Returns true if the compressor is still approaching a lookahead time specified before (smoothing is applied to the
