...
inline float FastClip(float x, float minval, float maxval) {
    return (fabsf(x - minval) - fabsf(x - maxval) + (minval + maxval)) * 0.5f;
}
...
template <typename T> class UnityComplexNumberT {
  public:
    // No constructor because we want to be able to define this inside anonymous unions (this is also why we don't use
    // std::complex<T> here)
    ...
    template <typename T1, typename T2, typename T3>
    inline static void Mul(const UnityComplexNumberT<T1>& a, const UnityComplexNumberT<T2>& b,
                           UnityComplexNumberT<T3>& result) {
    ...
    template <typename T1, typename T2, typename T3>
    inline static void Add(const UnityComplexNumberT<T1>& a, const UnityComplexNumberT<T2>& b,
                           UnityComplexNumberT<T3>& result) {
    ...
    template <typename T1, typename T2, typename T3>
    inline static void Sub(const UnityComplexNumberT<T1>& a, const UnityComplexNumberT<T2>& b,
                           UnityComplexNumberT<T3>& result) {
        result.re = static_cast<T1>(a.re - b.re);
        result.im = static_cast<T1>(a.im - b.im);
    }
    ...
    template <typename T1, typename T2, typename T3>
    inline static void MulAdd(const UnityComplexNumberT<T1>& a, const UnityComplexNumberT<T2>& b,
                              const UnityComplexNumberT<T2>& c, UnityComplexNumberT<T3>& result) {
        // Store temporarily in case a or b reference the same memory as result
        T3 t = a.re * b.im + a.im * b.re;
...
class HistoryBuffer {
  ...
  public:
    inline void Feed(float sample) {
        ...
        // The writeindex veriable may be read at the same time, so we don't want intermediate values indexing out of
        // the array.
        int w = writeindex + 1;
        if (w == length)
            w = 0;
...
template <const int _LENGTH, typename T = float> class RingBuffer {
  public:
    enum { LENGTH = _LENGTH };
...
class MutexScopeLock {
  public:
    MutexScopeLock(Mutex& _mutex, bool condition = true) : mutex(condition ? &_mutex : NULL) {
        if (mutex != NULL)
            mutex->Lock();
    }
    ~MutexScopeLock() {
...
void RegisterParameter(UnityAudioEffectDefinition& desc, const char* name, const char* unit, float minval, float maxval,
                       float defaultval, float displayscale, float displayexponent, int enumvalue,
                       const char* description = NULL);

void InitParametersFromDefinitions(InternalEffectDefinitionRegistrationCallback registereffectdefcallback,
                                   float* params);

void DeclareEffect(UnityAudioEffectDefinition& desc, const char* name, UnityAudioEffect_CreateCallback createcallback,
