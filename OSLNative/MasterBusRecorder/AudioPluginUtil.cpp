...
template <typename T> void UnitySwap(T& a, T& b) {
    T t = a;
    a = b;
    b = t;
}

template <typename T> static void FFTProcess(UnityComplexNumber* data, int numsamples, bool forward) {
...
HistoryBuffer::HistoryBuffer() : length(0), writeindex(0), data(NULL) {
}
...
void HistoryBuffer::ReadBuffer(float* buffer, int numsamplesTarget, int numsamplesSource, float offset) {
    ...
    int n, w = writeindex; // since ReadBuffer is called from the GUI thread, writeindex may be modified by the DSP
...
void RegisterParameter(UnityAudioEffectDefinition& definition, const char* name, const char* unit, float minval,
                       float maxval, float defaultval, float displayscale, float displayexponent, int enumvalue,
    ...
    definition.paramdefs[enumvalue].description = (description != NULL) ? strnew(description)
                                                  : (name != NULL)      ? strnew(name)
                                                                        : NULL;
...
void InitParametersFromDefinitions(InternalEffectDefinitionRegistrationCallback registereffectdefcallback,
    ...
    delete[] definition
        .paramdefs; // assumes that definition.paramdefs was allocated by registereffectdefcallback or is NULL
}

void DeclareEffect(UnityAudioEffectDefinition& definition, const char* name,
                   UnityAudioEffect_CreateCallback createcallback, UnityAudioEffect_ReleaseCallback releasecallback,
...
#define DECLARE_EFFECT(namestr, ns)                                                                                    \
    AudioPluginUtil::DeclareEffect(definition[numeffects++], namestr, ns::CreateCallback, ns::ReleaseCallback,         \
                                   ns::ProcessCallback, ns::SetFloatParameterCallback, ns::GetFloatParameterCallback,  \
                                   ns::GetFloatBufferCallback, ns::InternalRegisterEffectDefinition);

extern "C" UNITY_AUDIODSP_EXPORT_API int
    AUDIO_CALLING_CONVENTION UnityGetAudioEffectDefinitions(UnityAudioEffectDefinition*** definitionptr) {
        ...
        for (int test = 0; test < 2; test++) {
            ...
            for (int b = 4; b <= 20; b++) {
                ...
                printf("%2d bits: MaxErr=%15.8g ErrSum=%15.8g AvgErr=%15.8g ErrRMS=%15.8g [%s precision]\n", b, maxerr,
                       errsum, avgerr, rms, highprecision ? "high" : "low");
