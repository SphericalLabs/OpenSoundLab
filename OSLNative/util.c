...
/* Windows does not support C99 standard, so variable length arrays are not supported and are emulated using _malloca
 * and _freea. */
#include <malloc.h>
#include <stddef.h>
#endif
...
/* Unfortunately, the math functions of Ne10 are only available for armv7. The DSP functions are available for aarch64
 * too. To use Ne10, have to call ne10_init(); once to set the function pointers. */
#if __NE10
...
void _float32toint24buffer(float* src, int8_t* dest, int endianness, int n) {
    ...
    /// This does not work the way intended, probably bc of the stride, bc it worked when used on each sample
    /// individually...
...
#if __APPLE_VDSP_DONTUSE
    /* For small vector sizes (e.g. 1024), vDSP is slower here. But for large vector sizes (e.g.1024 << 8), it is a lot
     * faster. */
...
void _fAdd(float* src1, float* src2, float* dest, int n) {
    ...
    /* A bit slower than naive implementation (Ofast). Interestingly, the Neon version gets a speedup from O3 to Ofast,
     * while the naive implementation does not change. */
    if (n % 4 == 0) {
        float32x4_t temp1;
        float32x4_t temp2;
        for (int i = 0; i < n; i += 4) {
...
#if __APPLE_VDSP
    /* Interestingly, the vDSP version here is a little slower than the naive approach, also for large vector sizes
     * (e.g. 1024 << 8). */
    float result;
...
void _fInterleave(const float* src, float* dest, const int n, int channels) {
    ...
    DSPSplitComplex input = {.realp = src, .imagp = src + n / 2};
...
void _fDeinterleave(const float* src, float* dest, const int n, int channels) {
    ...
    DSPSplitComplex output = {.realp = dest, .imagp = dest + n / 2};
