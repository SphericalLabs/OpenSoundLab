...
usage() {
  cat <<'EOF'
Usage: scrcpy_wifi.zsh [--immediate | --wait-unplug] [--port N] [--bitrate Bps] [--crop WxH:x:y] [--angle deg] [--] [extra scrcpy args]

Examples:
  ./scrcpy_wifi.zsh
  ./scrcpy_wifi.zsh --immediate --port 5555 --bitrate 12M --crop 2064:2208:0:0 -- --turn-screen-off
...
main() {
  ...
  if [[ "$MODE" == "immediate" ]]; then
    if tcp="$(get_tcp_serial || true)"; [[ -n "${tcp:-}" ]]; then
      log "Found TCP/IP device: $tcp"
      pause_between_steps
  ...
  if [[ -z "${usb:-}" ]]; then
    log "No devices found. Plug in via USB or ensure Wireless debugging is paired."
    exit 1
  fi
