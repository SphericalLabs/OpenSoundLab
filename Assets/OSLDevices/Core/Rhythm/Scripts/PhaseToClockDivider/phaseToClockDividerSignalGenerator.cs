...
public class phaseToClockDividerSignalGenerator : signalGenerator
{
    ...
    const float phaseWrapHigh = 0.75f;
    const float phaseWrapLow = 0.25f;
    ...
    public override void processBufferImpl(float[] buffer, double dspTime, int channels)
    {
        ...
        for (int n = 0; n < buffer.Length; n += channels)
        {
            ...
            else
            {
                ...
                if (phaseDelta < 0f)
                {
                    // Negative phase movement can mean three different things:
                    // a reset forced phase back to 0, a real wrap crossed from the
                    // end of the cycle to the beginning or an external sync source
                    // made a small backward correction. Only the high-to-low case
                    // should advance the divider; small corrections must not look
                    // like an extra bar, especially in slow multi-cycle modes.
                    if (ignorePhaseDropAfterReset)
                    {
                        phaseDelta = 0f;
                        ignorePhaseDropAfterReset = false;
                    }
                    else if (lastPhaseSample > phaseWrapHigh && phaseSample < phaseWrapLow)
                    {
                        phaseDelta += 1f;
                    }
                    else
