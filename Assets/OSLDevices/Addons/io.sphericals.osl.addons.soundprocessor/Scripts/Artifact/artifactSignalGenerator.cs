...
namespace OpenSoundLab.Addons.SoundProcessors
{
public class artifactSignalGenerator : signalGenerator
{
    [DllImport("OSLNative")]
    public static extern void Artifact_Process(float[] buffer, float noiseAmount, int downsampleFactor, float jitterAmount, int bitReduction, int channels, int n);
    ...
    public override void processBufferImpl(float[] buffer, double dspTime, int channels)
    {
        ...
        if (input != null)
        {
            ...
            Artifact_Process(buffer, noise, dwnmpl, jitter, bitrdx, channels, buffer.Length);
        }
        if (!recursionCheckPre()) return; // checks and avoids fatal recursions
    }
}
