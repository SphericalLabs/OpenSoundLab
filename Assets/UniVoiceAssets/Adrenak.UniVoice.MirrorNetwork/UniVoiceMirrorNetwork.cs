...
namespace Adrenak.UniVoice.MirrorNetwork {
    public class UniVoiceMirrorNetwork : IChatroomNetwork {
        ...
        public UniVoiceMirrorNetwork() {
            // Unity update hook
            updateHook = UpdateHook.Create();
            updateHook.OnUpdate += OnUpdate;

            // Client joining and leaving a server
            NetworkManager.singleton.transport.OnClientConnected += Client_OnConnected;
            NetworkManager.singleton.transport.OnClientDisconnected += Client_OnDisconnected;

            // When a client joins and leaves the server
            NetworkManager.singleton.transport.OnServerConnected += Server_OnClientConnected;
            NetworkManager.singleton.transport.OnServerDisconnected += Server_OnClientDisconnected;

            // Processing messages on server and client.
            NetworkClient.RegisterHandler<UniVoiceMessage>(Client_OnMessage, false);
            NetworkServer.RegisterHandler<UniVoiceMessage>(Server_OnMessage, false);
        }
        ...
        async void SendToClient(short peerID, byte[] bytes, int delay = 0) {
            if (IsServerOrHost) {
                if (delay != 0)
        ...
        void Server_OnClientConnected(int connId) {
        ...
        void Server_OnClientDisconnected(int connId) {
        ...
        void Client_OnMessage(UniVoiceMessage message) {
        ...
        void Server_OnMessage(NetworkConnectionToClient connection, UniVoiceMessage message) {
